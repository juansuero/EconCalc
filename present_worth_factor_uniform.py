...
def calculate_present_worth_factor_uniform():
  """
  Calculates the present worth factor (uniform series) based on present value, interest rate, and number of years.
  ...
  try:
    ...
    present_value = annuity * (((1 + interest_rate) ** number_years) - 1) / (interest_rate*(1 + interest_rate) ** number_years)
    result_label.config(text=f"The Present Value is ${present_value:.2f}")  # Display factor, not annuity
  except ValueError:
    result_label.config(text="Invalid Input! Please enter numbers.")

# Create the main window
window = Tk()
window.title("Present Worth Factor (Uniform Series) Calculator")
...
# Button
calculate_button = Button(window, text="Calculate", command=calculate_present_worth_factor_uniform)
