...
def calculate_present_worth_factor():
  """
  Calculates the present worth factor based on future value, interest rate, and number of years.
  ...
  try:
    ...
    present_value = future_value / (1 + interest_rate) ** number_years
    result_label.config(text=f"The Present Value is ${present_value:.2f}")  # Display factor, not future value
  except ValueError:
    result_label.config(text="Invalid Input! Please enter numbers.")

# Create the main window
window = Tk()
window.title("Present Worth Factor Calculator")
...
# Button
calculate_button = Button(window, text="Calculate", command=calculate_present_worth_factor)
